...
args = c(
    "Data/Training_Data/Point_Extractions/", #1
    "MOD_CLASS", #2
...
trainingPath <- args[1]
predictClass <- args[2]
modelOutputPath <- args[3]


cat("these are the arguments: \n", 
    "- Path to training data files (should be multiple):", trainingPath, "\n",
    "- Class to predict on (should be MOD_CLASS or COARSE_CLASS)", predictClass, "\n",
    "- Path to model output", modelOutputPath
)
...
###################################################################################################
# === Load Data ===
list_of_pts_extracted_locs <- list.files(here(trainingPath), pattern = ".gpkg$", full.names = TRUE, recursive = FALSE)
...
target_var <- predictClass
if(target_var == "MOD_CLASS"){
    data <- data |> select(-starts_with("COARSE"))
...
#                                   .default = twi)
    # ) 
# st_write(data_sf, paste0("Data/Training_Data/Combined_Training_Datasets/", predictClass, "_sf_points_for_RFM_FieldLocs.gpkg"))
...
readr::write_csv(train_data, here("Data/Dataframes/RF_TrainingPoints.csv"))
readr::write_csv(test_data, here("Data/Dataframes/RF_TestPoints.csv"))
