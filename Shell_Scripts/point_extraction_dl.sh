...
#SBATCH --job-name=pt_extract
#SBATCH --ntasks=2
#SBATCH --ntasks-per-node=1
#SBATCH --output=Shell_Scripts/SLURM/slurm-pt_extract-%j.out
...
for number in "${include[@]}"; do
    ...
    srun --nodes=1 --ntasks=1 --exclusive \
        ...
        10 \
        "$number" >> "Shell_Scripts/logs/pt_extract_${number}_$(date +%Y%m%d).log" 2>&1 &
done

wait
echo "All point extraction jobs completed."
